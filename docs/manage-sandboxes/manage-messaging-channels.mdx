...
NemoClaw runs cleanup in running sandboxes through `openshell sandbox exec`.
A failed, cancelled, or inconclusive native command does not retry through the local runtime provider or SSH.
Cleanup succeeds only when the command exits successfully and returns its completion sentinel.

<AgentOnly variant="openclaw">
For OpenClaw WeChat only, NemoClaw checks the registered runtime provider for its distinct stopped-state cleanup before attempting native command execution.
