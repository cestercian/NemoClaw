...
| `brew-balanced` | The same Homebrew endpoints and binaries as `brew`, with GET and HEAD only for `raw.githubusercontent.com`. Balanced selects this preset; Open keeps `brew`. | Allows downloading and installing arbitrary Homebrew packages, which may contain malicious code. |
...
Removing the preset restores the reviewed GET-only baseline entry.
...
Its curl paths are used only by onboarding's post-create verifier.
...
Review capability ownership, resource limits, runtime identity, and image hardening in [Understand Process Controls](./security-controls/process-controls).
...
The `NEMOCLAW_EXPERIMENTAL=1` environment variable gates local NVIDIA NIM on eligible hosts other than native N1x FASTOS and generic Linux managed vLLM install/start.
Managed vLLM install/start is Docker-backed and requires Docker on `PATH`; attaching an already-running local vLLM server is separate.
DGX Spark and DGX Station managed vLLM entries appear by default when Docker is available.
Native N1x FASTOS omits local NVIDIA NIM and offers the Deferred managed-vLLM preview or explicitly selected standard onboarding.
Native N1x FASTOS remains outside the supported-platform set because its physical NemoClaw Express E2E validation is incomplete.
After NemoClaw qualifies the native N1x identity, you must provide explicit Express or standard-onboarding intent.
A qualifying Windows WSL N1x host uses managed llama.cpp through Windows Express.
The installer leaves provider and recipe selection to onboarding.
When no provider is requested or recorded and `NEMOCLAW_MODEL` is unset, onboarding selects managed llama.cpp; an optional compatible `NEMOCLAW_LLAMACPP_RECIPE` value makes the managed recipe explicit.
Onboarding recognizes one proof-backed GPU only when its normalized NVIDIA identity is either `NVIDIA RTX Spark N1X` or `NVIDIA RTX Spark N1X (6144-core Blackwell RTX GPU)`; the Windows chassis product name does not participate in selection.
Qualification also requires Linux Arm64 WSL, local Docker Desktop on the default local Docker context, at least 48,000 MiB of Docker and GPU memory, driver version `580.65.06` or later, Docker storage and runtime readiness, NVIDIA GPU integration, and a successful Docker Desktop GPU passthrough proof.
...
Other Windows WSL Express installs also use WSL-local Ollama.
Windows-host Ollama remains available through manual onboarding when Docker Desktop WSL integration is enabled, `DOCKER_HOST` is unset, Docker uses the local `default` context, and Ollama has a loopback-only listener that is reachable from Docker and rejects unrecognized HTTP `Host` headers.
An already-running vLLM server on `localhost:${NEMOCLAW_VLLM_PORT:-8000}` appears in standard onboarding because selecting it is an explicit user action.
Native N1x requires you to decline or disable Express or set `NEMOCLAW_PROVIDER=vllm` first.

| Aspect | Detail |
|---|---|
| Default | Local NVIDIA NIM and generic Linux managed vLLM install/start are hidden. When Docker is available, DGX Spark and DGX Station managed vLLM entries are offered by default. Native N1x separately offers the Deferred managed-vLLM preview; its standard onboarding offers already-running vLLM on `localhost:${NEMOCLAW_VLLM_PORT:-8000}` when detected and requires explicit standard-onboarding intent. Qualifying Windows WSL N1x Express runs use managed llama.cpp. |
| What you can change | On eligible hosts other than native N1x FASTOS, set `NEMOCLAW_EXPERIMENTAL=1` before onboarding to surface Local NIM and, when Docker is available on `PATH`, generic Linux managed vLLM. To request only the Docker-backed managed vLLM path non-interactively, set `NEMOCLAW_PROVIDER=install-vllm`. |
| Risk if selected | NemoClaw has not fully validated these providers. NIM requires a NIM-capable GPU. The managed vLLM path requires Docker, pulls a container image, and starts it on a supported NVIDIA GPU host. Misconfiguration can cause failed inference or unexpected behavior. |
...
| Adding an entrypoint capability layer | OpenShell owns child capability enforcement for managed sandboxes. A second entrypoint policy can drift from exec and connect behavior. | Keep OpenShell enforcement enabled. For direct container launches, make the container runtime the owner and pass `--cap-drop=ALL`. |
...
| Assuming the retired device-auth bypass disables pairing | OpenClaw 2026.9.1 ignores the legacy `NEMOCLAW_DISABLE_DEVICE_AUTH` compatibility input, so relying on it can leave an operator unable to connect and mistaken about the active authentication controls. | Complete the normal device-pairing flow for every browser or CLI device. Use loopback access or SSH port forwarding for remote dashboards. |
...
| Same-UID native lifecycle in the OpenShell-managed topology | The native supervisor, gateway, and agent share the sandbox UID. OpenShell contains the sandbox, but a same-user agent can signal or imitate peer processes. NemoClaw's readiness and health checks prove service availability, not process provenance. | Keep the managed image under OpenShell. Use a direct root-entrypoint deployment outside OpenShell when separate gateway and agent UIDs are required. Recreate a sandbox after suspected same-UID compromise. |
</AgentOnly>
