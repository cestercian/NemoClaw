...
| Homebrew packages | `brew-balanced` for Balanced; `brew` for Open or explicit broader access | OpenClaw and Hermes |
...
| Homebrew packages | `brew-balanced` for Balanced; `brew` for Open or explicit broader access |
| Hugging Face model or dataset access | `huggingface` |

Balanced already includes `brew-balanced`, so it does not need the `brew` additions below.
Those commands opt into unrestricted raw GitHub methods; Open uses that broader preset by default.
Existing `brew` grants remain active when you reselect Balanced defaults.
To narrow an existing grant, use the [Homebrew preset replacement procedure](#homebrew-specifics) below.
...
The sandbox base image includes Homebrew (Linuxbrew).
Balanced supplies its network access through `brew-balanced`; Open supplies it through `brew`.
Adding `brew-balanced` alone does not narrow an existing `brew` grant.
Balanced defaults preserve existing grants. To explicitly narrow an older `brew` grant, remove it before adding the restricted preset:

```bash
$$nemoclaw my-assistant policy remove brew --yes &&
  $$nemoclaw my-assistant policy add brew-balanced --yes
```

Verify that `policy list` shows `● brew-balanced` and `○ brew`.
If the addition fails, resolve the reported error and rerun the add command.
Keep `brew` on Open or when you intend its broader access.
Use the broader `brew` command below only when you intend to allow unrestricted raw GitHub methods.
