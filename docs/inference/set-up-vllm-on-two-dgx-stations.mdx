...
When Express recognizes an existing legacy single-Station workload, that workload must remain unchanged during peer discovery.
If it changes, the installer stops and reports the image and ownership-contract mismatch instead of continuing the upgrade.
Inspect the `nemoclaw-vllm` container with `docker inspect nemoclaw-vllm`.
Restore the original single-Station workload before retrying, or stop the upgrade if the change was intentional.
