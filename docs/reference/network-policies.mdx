...
| `openclaw_api` | `openclaw.ai:443`, `catalog.openclaw.ai:443` | `/usr/local/bin/openclaw`, `/usr/local/bin/node` | GET and POST to `openclaw.ai`; GET only to `catalog.openclaw.ai` |
...
Balanced uses `brew-balanced` to allow only GET and HEAD to `raw.githubusercontent.com` for the preset's binaries.
Open retains unrestricted raw GitHub access through `brew`, and Personal retains its broad web policy.
Reselecting Balanced's suggested presets preserves an existing `brew` grant, including its unrestricted raw GitHub access.
Adding `brew-balanced` does not narrow that broader grant.
To narrow it, explicitly remove `brew` and apply `brew-balanced`.
Resuming onboarding reapplies the saved selection; skipping optional setup preserves existing grants.
