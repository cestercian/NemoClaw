...
Put the sandbox name before a sandbox action, for example `$$nemoclaw <name> policy list`.
...
For pending setup, wait for onboarding to finish or use `$$nemoclaw onboard --resume` after an interruption.
A registered sandbox can share an action name; `$$nemoclaw doctor status` addresses a sandbox named `doctor`.
The global `$$nemoclaw doctor` command remains available without a sandbox name.
...
If NVIDIA inference credentials are absent and no sandbox is registered, the installer installs NemoClaw and skips onboarding.
It does not create an OpenShell provider or sandbox, and it does not report onboarding as complete.
If a supported credential or registered sandbox exists, the installer follows the normal onboarding or recovery path.
An invalid credential fails validation and does not select deferred onboarding.

After the runtime process supplies one of these credentials, run the onboarding command for the selected agent:

- Non-empty `NVIDIA_INFERENCE_API_KEY`
- Non-empty `NVIDIA_API_KEY`
- `NEMOCLAW_PROVIDER_KEY` containing a credential value, rather than a provider selector such as `build` or `routed`

Credential presence selects normal onboarding; that path still validates the value.

For the selected agent, run:

- Hermes: `nemohermes onboard`
- LangChain Deep Agents Code: `nemo-deepagents onboard`
...
Connect an external Agent Client Protocol (ACP) client to a managed Hermes sandbox.
The adapter sends ACP traffic through standard input and standard output, and writes diagnostics to standard error.

```bash
nemoclaw-acp --sandbox <name> [--gateway <name>] [--timeout <seconds>]
```

`--sandbox` is required.
When the sandbox name is registered on more than one gateway, pass `--gateway <name>` to select its owning NemoClaw gateway.
`--timeout` ends the ACP session after the specified positive number of seconds, up to `86,400`.

Before the adapter starts the session, it revalidates the registered sandbox and its ready state under the sandbox lifecycle lock.
It also revalidates the sandbox lifecycle generation, identity, and OpenShell gateway.
It can select and, when required, start only that exact gateway when it is missing, unhealthy, or unreachable, or when a different gateway is connected.
A conflicting or ambiguous gateway identity, authentication, schema, timeout, or other observation failure stops the adapter before gateway startup or session creation.
...
The command reads the provider endpoint from the sandbox's OpenShell gateway and compares it with the registered endpoint.
For NVIDIA Endpoints, it verifies the gateway's built-in NVIDIA provider profile and default endpoint.
A custom NVIDIA provider profile or endpoint override cannot be included in an export.
If OpenShell cannot supply the required evidence, the command stops without writing output.
...
| `--name <name>` | Set `metadata.name`. Use at most 40 lowercase letters, digits, or hyphens, starting with a letter. |
...
On Linux, OpenClaw and Hermes can register one experimental external host component during fresh onboarding with a NemoClaw-managed Docker-driver gateway.
Create the declaration before onboarding and use a new explicit sandbox name.
NemoClaw rejects resume, recreation, reuse, repair, and externally supervised gateways.
For the declaration, fixed interceptor settings, activation protocol, incomplete-state handling, and credential boundary, refer to [Register an External Component During Onboarding](../deployment/register-external-component).
...
```bash
$$nemoclaw onboard --profile llama-cpp.dgx-spark-gb10.single.muse-glimmer-30b
```
...
OpenClaw onboarding does not enter the `complete` state until NemoClaw proves that the local CLI operator pairing is settled. Onboarding first observes the canonical local CLI device. If the device is pairing-only, the host runs one bounded request producer on the owning gateway. The in-sandbox watcher is the only component that approves the onboarding upgrade. The host observes through any same-device pending state and verifies the final settled state without sending an approval.
...
During finalization, NemoClaw stops onboarding when it cannot complete the process check or a required secret-boundary check refuses.
It does not report the sandbox as ready.
Run `$$nemoclaw <sandbox-name> doctor`, correct the reported condition, then run `$$nemoclaw onboard --resume`.
...
Run `$$nemoclaw <sandbox-name> destroy` to check retained recovery and reconcile verified residual state.

Use the result from `destroy` to choose the next action:

- If OpenShell reports the sandbox present, `destroy` preserves the record and removes no resources. OpenShell exposes no atomic delete-by-identity primitive. Neither NemoClaw nor manual inspection can bind a later mutable-name delete to the durable identity fingerprint. You can inspect the owning gateway for diagnosis, but do not delete the sandbox by mutable name. Live retained-sandbox removal is not supported yet. Use a different explicit sandbox name while this record remains blocked.
- If OpenShell cannot determine whether the sandbox is present, `destroy` reports unknown presence, removes no resources, and preserves the recovery record. Correct the OpenShell gateway or list failure, then rerun `destroy`.
- If OpenShell confirms the sandbox is absent, `destroy` selects one retained record from its registry generation, gateway, create-attempt evidence, and available identity fingerprint.
  Before pending identity publication, it can instead select one record with a durable identity fingerprint when the pending reservation belongs to the recorded onboarding session, the saved session recovery evidence matches the record, and the registry has no lifecycle identity evidence.
  Provider and Docker cleanup then apply their own immutable identity checks.
  `destroy` removes only qualified residual resources and clears the matching recovery record only after verified cleanup.
...
This fail-closed record keeps only the affected sandbox name unavailable. This command does not accept externally supplied identity authority.
...
Before reserving an inference route, onboarding can release a foreign-session route-only reservation while it holds the onboarding lock.
For an already published sandbox on the gateway selected in the current session's checkpoint, it can transfer a foreign pending reservation to the new session while retaining the sandbox data.
The row remains pending until normal onboarding finalization, so reclaiming it does not publish an unverified inference route.
It preserves the current session's reservation and any row that has gained verified sandbox-create authority.
A published row remains unchanged when its gateway differs from the selected authority or that authority is unavailable.
Without the lock, it preserves the foreign reservation.
...
Follow the [retained-sandbox recovery procedure](#inspect-retained-sandbox-recovery); its OpenShell-absence and immutable runtime-identity requirements also apply to APF creation. For Docker-backed APF creation, its Docker container-identity requirement also applies.
...
retire_legacy_forwards_flag=--retire-legacy-forwards
NEMOCLAW_GATEWAY_PORT="$selected_port" NEMOCLAW_REQUIRE_ALL_SANDBOX_BACKUPS=1 $$nemoclaw backup-all "$retire_legacy_forwards_flag" && \
...
During Windows Express, onboarding recognizes one proof-backed GPU only when its normalized NVIDIA identity is either `NVIDIA RTX Spark N1X` or `NVIDIA RTX Spark N1X (6144-core Blackwell RTX GPU)`, verifies the remaining N1x readiness requirements, and automatically selects the Experimental managed Qwen 3.6 recipe.
...
If you cancel a brand-new onboarding run at the policy-tier selector or either policy-preset selector after sandbox creation, NemoClaw preserves the incomplete sandbox, registry entry, and onboarding session for retained recovery.
...
Follow the [retained-sandbox recovery procedure](#inspect-retained-sandbox-recovery) to reconcile this cancellation.
...
- The recorded inference selection matches the live route, and every configured route serves a valid response to a bounded request for the recorded model.
  The models probe must return HTTP 2xx, except that `openrouter-api` can return HTTP 404 for `GET /v1/models`.
  A failed model request rejects readiness before the agent starts.
...
For an older OpenClaw registration without recorded lifecycle identity, `launch` requires a ready live sandbox with an observable identity before it starts the agent.
If this identity check fails, `launch` stops before starting the agent.
Run `$$nemoclaw <name> doctor`, resolve the reported issue, and retry `$$nemoclaw launch <name>`.
...
Run `$$nemoclaw <name> doctor` to diagnose a failed model request.
</AgentOnly>
...
- `lifecycle` verifies or recovers the selected runtime lifecycle.
...
When the captured output reports that the turn's deadline fired, the wrapper replays the partial output and writes deadline guidance to `stderr`. It exits with status `1` instead of the upstream status `0`. The diagnostic shell-quotes the sandbox name and forwarded arguments, then redacts detected credential values before writing the recovery command to `stderr`. If redaction changes the recovery command, the diagnostic tells you not to replay it; otherwise, it labels the command as runnable inside the sandbox. For a registered sandbox, both captured paths pin the sandbox's recorded gateway with an explicit `-g`.

Neither path forwards an interactive terminal on `stdin`.
When NemoClaw recognizes `-m` or `--message`, it closes the OpenShell child's `stdin` before dispatch.
This prevents an unused pipe from delaying execution while OpenShell waits for EOF.
Nonempty `--message-file` arguments and calls without a recognized inline message preserve a pipe or redirect.
...
Restore sandbox readiness and re-establish host-side forwards without opening an SSH session.
Use this after a sandbox crash or when `$$nemoclaw <name> status` reports a stopped sandbox or an unhealthy forward.
...
After the sandbox is available, `recover` sends authenticated requests only to repair supported in-sandbox agent processes and host forwards. Standard sandbox start and stop remain OpenShell-owned.
...
It does not use ordinary `openshell sandbox exec`, manual in-sandbox relaunch, or container replacement as a fallback.
...
If the gateway is healthy, `recover` checks or repairs host-side forwards.
It exits zero only when the required forward checks succeed.
When the dashboard port is held by a listener that NemoClaw cannot attribute to the sandbox's own OpenShell forward, the command reports the port, leaves that listener running, does not restore a forward onto it, and exits nonzero.
Legacy SSH forward records do not establish ownership.
Stop the owning OpenShell gateway or service to release the listener, then run `recover` again.
...
Request a native restart of the supported in-sandbox gateway.
Use this after runtime configuration or plugin changes that the agent reads only at gateway startup.
...
For OpenClaw, NemoClaw waits for `/readyz` to return HTTP `200` with JSON `{"ready":true}` before it repairs host-side forwards.
...
NemoClaw invokes the agent's native lifecycle command through the selected OpenShell sandbox.
OpenShell owns sandbox containment, and the agent runtime owns the gateway and its child processes.
NemoClaw does not signal, authorize, or respawn an agent-owned process.
...
The managed nonroot supervisor continuously repairs those processes and stops an alive but deaf gateway after four consecutive failed health checks.
If the gateway requests a service-managed restart with status 75 five times within 60 seconds, the supervisor stops before a sixth launch.
Clean exits and status 79 wait for authenticated recovery, while other exit statuses fail immediately.
If the Hermes restart closes the OpenShell exec relay before it returns status, NemoClaw waits for the selected sandbox control plane to become ready again.

</AgentOnly>

<AgentOnly variant="openclaw,hermes">

For OpenClaw, NemoClaw verifies the readiness endpoint before it repairs only the host-side OpenShell forwards.
For Hermes, NemoClaw verifies the agent health endpoint before it repairs only the host-side OpenShell forwards.

</AgentOnly>

<AgentOnly variant="hermes">

For Hermes, the native restart path validates `/sandbox/.hermes/.env` against the secret-boundary guard and validates the supervisor runtime environment.
The native `mcp_servers` content remains authoritative, including direct edits.
The supervisor does not reconcile it against host-side MCP state.
Use supported host commands such as `$$nemoclaw <name> config set` and `$$nemoclaw inference set` for settings that must match host registry state.
The command reports `secret-boundary refusal` when the environment does not pass that guard.

</AgentOnly>

<AgentOnly variant="openclaw,hermes">

The command reports the failure layer returned by the native agent command.
Host-side failures include unsupported agent, native agent command, health timeout, and forward recovery failure.
Inspect the native diagnostic before choosing a repair.
For ordinary process or health failures, correct the cause, then run `$$nemoclaw <name> stop` and `$$nemoclaw <name> start` to reset the native supervisor.
...
Hermes can also report secret-boundary refusal when the native command reports an unsafe runtime directory.

</AgentOnly>

<AgentOnly variant="hermes">

For Hermes, `HERMES_RUNTIME_PREPARATION_FAILED stage=<stage>` identifies a supported input that still failed after five bounded preparation attempts. Correct the named input through its supported host-side configuration or credential flow, then run `$$nemoclaw <name> stop` followed by `$$nemoclaw <name> start`. If startup layout repair reports the maximum log-entry count or repair depth, archive or remove old retained logs from a trusted host-side recovery environment, then stop and start the sandbox. For symbolic links, hard links, unsafe entry types, or paths that changed during repair, restore a trusted snapshot into a recreated sandbox or recreate from host-side onboarding configuration. Rebuild for an integrity mismatch or when the input or sandbox state cannot be repaired safely.

</AgentOnly>

<AgentOnly variant="openclaw,hermes">

An older image without the current native lifecycle command requires `$$nemoclaw <name> rebuild --yes`.
For gateway restart, NemoClaw does not fall back to direct provider-container mutation or manual process signaling.
...
<AgentOnly variant="openclaw,hermes">
  After OpenShell confirms the sandbox stopped, NemoClaw attempts to stop that sandbox's
  host dashboard forward. If the sandbox does not stop, NemoClaw leaves the dashboard forward
  running.
</AgentOnly>
The shared host gateway, tunnel services, and any local NIM inference container serve other
sandboxes and keep running.
<AgentOnly variant="openclaw,hermes">
  Stopping an already-stopped sandbox succeeds. NemoClaw also attempts
  to remove any leftover dashboard forward for that sandbox.
</AgentOnly>
<AgentOnly variant="deepagents">
  Stopping an already-stopped sandbox succeeds without changes.
</AgentOnly>
The standard Docker and native Podman paths require a registered immutable sandbox identity and a reachable owning OpenShell gateway. NemoClaw does not fall back to direct provider-container mutation. Portable remains a separate receipt-bound lifecycle path.

### `$$nemoclaw <name> start`

Ask OpenShell to start a sandbox stopped with [`$$nemoclaw <name> stop`](#$$nemoclaw-name-stop) or by a host reboot, then repair the in-sandbox gateway and host-side forwards the same way [`$$nemoclaw <name> recover`](#$$nemoclaw-name-recover) does.
...
Repair write access to NemoClaw's local state, then retry `$$nemoclaw <sandbox-name> status` before running another lifecycle command.
...
When the existing config guard verifies a same-user nonroot launch, `/sandbox/.openclaw` uses `0700` and `openclaw.json` uses `0600`.
Separate gateway identities and unverified layouts retain `2770/0660` for shared access.
If `openclaw doctor --fix` removes required group access, `doctor` reports a `Config permissions` warning.
Pass `--fix` to restore the permissions selected by the existing guard without rebuilding.
Restarting the sandbox repairs the same drift when the config tree passes its safety checks.
...
| `--fix` | Restore the mutable OpenClaw config permission contract. Mutually exclusive with `--json` |
...
<AgentOnly variant="openclaw,hermes">
When `--since` is not set, the command reads both agent gateway output and OpenShell audit events, so policy denials appear alongside the gateway log stream.
With `--since`, the command reads only OpenShell logs.
If one log source is unavailable, NemoClaw prints a warning and keeps reading the remaining source.
</AgentOnly>
<AgentOnly variant="deepagents">
The command reads OpenShell logs, including audit events, so policy denials appear with other OpenShell log entries.
</AgentOnly>
If NemoClaw cannot enable OpenShell audit logs, it prints a warning and policy-denial events can be missing.

<AgentOnly variant="openclaw">
NemoClaw labels nonempty OpenClaw gateway log lines with `[gateway]` after any recognized leading timestamp.
It preserves an existing label and leaves OpenShell audit output unchanged.
</AgentOnly>
...
Stop managed local inference resources, remove runtime-provider-owned resources created during onboarding, and delete the sandbox. This removes the sandbox from the registry. NemoClaw resolves the registry that owns the sandbox before it starts cleanup, even when another local OpenShell gateway is active. For Ollama-backed sandboxes, `destroy` also asks Ollama to unload currently loaded models and clears stale auth proxy state on a best-effort basis. For Model Router sandboxes, `destroy` keeps the process and recovery identity when another sandbox uses the port or when session, process, or absence checks are inconclusive. It also preserves a replacement onboarding session when the captured session identity changed. If the captured session uses the destroyed sandbox name with another router port, `destroy` clears only the sandbox association and preserves that router's recovery identity. For lock order, same-port peer handling, and cleanup checks, refer to [Set Up Model Router](../inference/hosted-inference/set-up-model-router#router-and-sandbox-lifecycle-locks).
...
NemoClaw stops before provider cleanup and sandbox deletion and preserves the local registry entry.
...
Repeating a successful add with identical inputs is idempotent.
A failed re-add can leave residual gateway-provider state when prior refresh material cannot be restored.
...
NemoClaw validates the complete checked-in profile contract, provider type, and credential keys before it reuses a provider.
For an existing refresh provider with matching metadata, it preserves the working credential until OpenShell mints the replacement.
A later refresh failure can still leave changed gateway refresh material; follow the rollback guidance below.
A missing provider is created when complete credentials are available.
OpenShell provider inspection uncertainty stops the operation before provider mutation.
On a fresh add, NemoClaw may already have applied the credential-free policy preset and attempts to remove it before exiting.
If that policy cleanup fails, rebuild the sandbox first, then run the `$$nemoclaw <name> policy remove <channel>` command printed by the CLI.
With complete credentials, NemoClaw replaces a mismatched provider only when it is unattached or attached exclusively to the selected sandbox.
An attachment to another sandbox stops replacement.
The `--force` flag does not override this provider attachment authority.
...
It prints the exact effective egress scope when the preset would open or replace access, or reports that no new egress would be opened when the preset is already effective.
...
When final policy application fails after provider registration during a fresh add, NemoClaw attempts to roll back the bridge providers, preset, and staged environment credentials before registry state is written, then exits without prompting for a rebuild.
If any gateway-side step fails, rollback continues and prints a `Rollback could not fully clean <surfaces>` warning so the operator can clean up manually.
When the same failure happens on a re-add of an already-enabled channel, NemoClaw restores the prior `messagingChannels` entry, staged environment credentials when available, and registry credential hashes.
An existing provider can be changed during re-add.
NemoClaw attempts to restore stored provider credentials, but it cannot restore prior gateway refresh material that was available only during the earlier setup command.
It flags `gateway-providers` as residual.
Inspect the channel's gateway providers, restore the preset YAML, and correct the gateway failure.
When the gateway is reachable, run the `$$nemoclaw <name> channels remove <channel>` command printed by the CLI, then rerun `$$nemoclaw <name> channels add <channel>`.
After a successful rebuild, `channels add` verifies that the selected Telegram, Discord, or Slack bridge starts and reports credential, startup, or plugin discovery warnings.

For OpenClaw, an official-plugin provenance failure stops the image build before those runtime checks.
Report the failure diagnostic, plugin name and NemoClaw version to a maintainer; NemoClaw manages the package pins and build cache.
...
Clear the stored credentials for a messaging channel and rebuild the sandbox so the image drops the channel. Running `remove` for a channel that was never configured is a no-op against the credentials file and normally still triggers the rebuild prompt. OpenClaw WeChat also checks for orphaned physical session state when no channel or policy entry exists; if cleanup cannot be confirmed, removal stops before rebuilding. When the bridge provider is attached to a live sandbox, NemoClaw detaches it before deleting the provider from the OpenShell gateway. If deletion fails, NemoClaw attempts to restore the attachment before returning the failure. If restoration also fails, the diagnostic names the provider and sandbox and warns that channel delivery may remain interrupted. If the matching built-in policy preset is applied, NemoClaw also removes that preset from the current OpenShell policy so the upstream API is no longer allow-listed after the channel is gone. When removing Teams, NemoClaw removes Teams-owned Microsoft login credential bindings from matching Outlook policy endpoints. It refuses to remove a binding owned by another integration. No session preset list exists to synchronize.
...
NemoClaw runs cleanup for a running sandbox through native OpenShell execution.
Cleanup succeeds only when the command exits successfully and returns its completion sentinel.
If execution fails or the result is inconclusive, the command refuses to rebuild and prints recovery guidance.
Fix the reported condition, then rerun the command so a rebuild cannot restore stale channel authentication files.

For OpenClaw WeChat only, NemoClaw checks the registered runtime provider for its distinct stopped-state cleanup before native execution.
The provider must confirm sandbox ownership, a stopped runtime resource, and one writable state resource containing the declared paths.
If the registry entry is missing, the provider does not support stopped-state cleanup, or no eligible stopped runtime resource exists, NemoClaw falls through to native cleanup.
Removal proceeds only when that command exits successfully and returns its completion sentinel.
...
Hermes includes the stable credential handle from the readiness check in the resolver placeholder and preserves it through inspection, rollback, and lifecycle reconciliation while OpenShell still reports it.

</AgentOnly>
<AgentOnly variant="openclaw">

OpenClaw MCP add, restart, and remove mutate the native `openclaw.json` `mcp.servers` map under the writable state root.
OpenClaw MCP update changes only the live OpenShell policy.
...
```bash
$$nemoclaw my-assistant mcp migrate
$$nemoclaw my-assistant mcp migrate --apply
```

| Flag | Description |
| --- | --- |
| `--apply` | Authorize the native write, verification, legacy cleanup, and retirement of legacy registry fields |
| `--json` | Emit the migration plan or result as JSON without credential values |

### `$$nemoclaw <name> mcp update`

Replace the complete denied-tool list for one current managed MCP server without changing its agent-native endpoint, credential reference, provider, attachment, or agent adapter.
...
Run `$$nemoclaw my-assistant mcp migrate --apply` when you intend to convert those registrations to native configuration, or explicitly resolve the registry-only rows first.
...
| Flag | Description |
| --- | --- |
| `--force` | Remove a modified same-name native agent entry and continue conservative policy and attachment cleanup; never delete the workspace provider |
...
OpenClaw plugins are a different kind of extension. To install an OpenClaw plugin, refer to [Install OpenClaw Plugins](../manage-sandboxes/install-openclaw-plugins). Skill installation invokes the unmodified `openclaw skills install <staged-path> --agent main --force` command. The pinned native command targets OpenClaw's `main` agent and allows OpenClaw to replace a same-name skill. OpenClaw owns discovery, precedence, and session activation.

</AgentOnly>
<AgentOnly variant="hermes">

Hermes plugins are different from agent skills. Because the pinned Hermes CLI has no native local-add verb, NemoClaw places the skill only at `/sandbox/.hermes/skills/<name>`. Hermes owns discovery and session activation; use `skill list` and a new Hermes session to observe the result.

</AgentOnly>
<AgentOnly variant="deepagents">

Because the pinned Deep Agents Code CLI has no native local-add verb, NemoClaw places the skill only at `/sandbox/.deepagents/agent/skills/<name>`. It does not inspect, mirror, or remove same-name project or legacy copies. Deep Agents Code owns discovery, precedence, and session activation; use `skill list` and a new DCode session to observe the result.
...
```bash
$$nemoclaw my-assistant skill list
```
...
The pinned OpenClaw CLI has no native remove verb. NemoClaw deletes only `/sandbox/.openclaw/workspace/skills/<name>`. A same-name bundled, project, plugin, global, or higher-precedence copy is untouched.

</AgentOnly>
<AgentOnly variant="hermes">

The pinned Hermes `skills uninstall` verb applies only to hub-installed skills and is not equivalent for a locally placed tree. NemoClaw therefore deletes only `/sandbox/.hermes/skills/<name>`.
...
The command enumerates the session store through `openclaw sessions list --agent <id> --json` and copies only matching `<sessionId>.jsonl` files and optional `<sessionId>.trajectory.jsonl` files.
It never picks up `sessions.json`, stale `.jsonl.lock` files, or other store bookkeeping.
For a registered sandbox, session lookup, transfer, and staging cleanup use its recorded OpenShell gateway.
NemoClaw reports success only after the OpenShell transfer process, artifact verification, host publication, transfer-process cleanup, and lifecycle-lock release.
...
With no positional keys, the command exports every non-internal session for the agent; if only internal warm-up sessions exist, it writes no artifact.
...
For a registered sandbox, export, transfer, and staging cleanup use its recorded OpenShell gateway.
NemoClaw reports success only after the OpenShell transfer process, artifact verification, host publication, transfer-process cleanup, and lifecycle-lock release.
Hermes stores session history in a SQLite database, so the command refuses positional keys, `--format tar`, and `--include-trajectory` with a clear error.
...
Upgrade a sandbox to the current agent version while preserving workspace state. The command backs up workspace state, captures the current OpenShell policy into a private temporary handoff, destroys the old sandbox, recreates it with the current image, and restores workspace state.
...
Prepared backup recovery for a legacy upgrade reuses the digest-verified pre-upgrade policy handoff instead.
It stops before deletion if that handoff is missing or changed.

</AgentOnly>

Credentials are stripped from backups before storage.
The replacement receives the captured OpenShell document directly; NemoClaw does not reconstruct policy from preset records.
The replacement uses the recorded compatible-endpoint reasoning mode, reasoning effort, web search selection, and serving-profile provenance instead of ambient shell values.
...
<AgentOnly variant="hermes">
Rebuild preserves supported values recorded through `config set`, excluding gateway settings, managed inference-route fields, and credential material.
Refer to [Rebuild While Preserving State](../manage-sandboxes/operate-sandboxes/recover-and-rebuild-sandboxes#rebuild-while-preserving-state) for capture, verification, and failure handling.
</AgentOnly>
...
| `--force` | Skip the confirmation prompt. It does not bypass an unavailable MCP source or allow rebuild to continue when a manifest-declared state directory or state file cannot be fully backed up, when the selected remote provider (other than Hermes Provider) no longer exposes the recorded credential key, when that credential is expired, or when its expiration metadata cannot be verified. A present credential key without an expiration entry is treated as non-expiring. |
...
For a managed Docker OpenClaw sandbox in `Error`, `rebuild` captures declared state without starting or executing in the stopped source. The original container must still exist, and any mounted managed state volume must have the expected ownership and be used only by that container. Capture or identity validation failure stops replacement before deletion. User-managed files, including `.env` and `.mcp.json`, are excluded; keep secure host copies and add them again after recovery. See [Rebuild While Preserving State](../manage-sandboxes/operate-sandboxes/recover-and-rebuild-sandboxes#rebuild-while-preserving-state) for preservation limits and recovery steps.
...
| Flag | Description |
| --- | --- |
| `--check` | Inspect sandbox state without rebuilding. Exits nonzero when it reports stale state, an unknown version, a backup recovery condition, a sandbox missing from its recorded gateway, or a Ready sandbox that retains intentional-stop state. Inspect each diagnostic before you select a remediation. |
| `--auto` | Rebuild eligible stale sandboxes and reconcile retained intentional-stop state without prompting. Used by the installer to upgrade in place. |
| `--yes`, `-y` | Skip confirmation prompts for rebuilds and stopped-state reconciliation. |
...
Rerun `$$nemoclaw upgrade-sandboxes --auto` to reconcile the stopped state without rebuilding a current replacement again.
The retry is complete when the command exits zero and reports that the sandbox was reconciled to stopped state; `$$nemoclaw upgrade-sandboxes --check` should then omit that sandbox from the reconciliation plan.
...
`--to <dst>` is not available for a snapshot of a sandbox that uses a NemoClaw-managed image.
NemoClaw stops before it creates, deletes, or changes `dst`.
When `dst` does not exist, it is auto-created from the source image.
Every source registry entry must record a complete durable inference route.
For a Docker- or VM-driver source, the source can be stopped when its registry entry also records the sandbox image.
...
If NemoClaw cannot confirm the same valid destination identity after a clone create-attempt, it prints a create-attempt label.
If NemoClaw confirms the destination identity but cannot register or publish it, the command exits nonzero before state restore.
If the failure reports that the pending clone changed during registration, NemoClaw preserves the current registry row.
Recovery depends on the reported state. Follow [Recover a Failed Cross-Sandbox Clone](../manage-sandboxes/state-and-backups/create-and-restore-snapshots#recover-a-failed-cross-sandbox-clone) for every failed cross-sandbox clone before retrying or deleting a destination.
...
Use `$$nemoclaw <name> status` for the sandbox's full live health and recovery details.
...
Managed providers keep the existing provider-and-model output. Compatible providers add the endpoint only when published same-gateway metadata matches the live route and is safe to display; otherwise the command keeps the live provider and model and returns credential-free recovery information. Refer to [View the Active Inference Route](../inference/manage-inference/view-active-inference-route) for the exact text and JSON fields, omission states, safety rules, and recovery procedures.
...
Before changing the route, NemoClaw compares it with every same-gateway registry entry, including stopped sandboxes.
When only the provider or model differs, the command names the affected sandboxes and warns immediately before it re-points the shared route.
Those sandboxes use the new live route until `connect`, onboarding, or another `inference set` command re-points it.

For the same provider name, custom compatible routes must have matching normalized endpoint URLs and API families.
Provider-global credential environment-variable names must also match.
Endpoint, API-family, credential, incomplete-route, and invalid-gateway-binding conflicts remain fail-closed.
The command exits nonzero before changing the OpenShell route, agent config, or host registry and names the conflicting sandboxes.
Align the provider-global configuration, repair incomplete metadata, or remove a conflicting sandbox that you no longer need.

Onboarding, `connect`, and `inference set` can time-share compatible provider and model routes without replacing provider-global configuration. Refer to [Use Shared Gateway Routes](../inference/manage-inference/use-shared-gateway-routes) for route-change warnings, compatibility fields, and the status drift report.
...
Canonical supported provider IDs are `nvidia-prod`, `nvidia-nim`, `nvidia-router`, `openai-api`, `openrouter-api`, `anthropic-prod`, `compatible-anthropic-endpoint`, `gemini-api`, `compatible-endpoint`, `hermes-provider`, `llama-cpp-local`, `ollama-local`, and `vllm-local`.
The installer alias `llama-cpp` is accepted and normalized to `llama-cpp-local`.
...
Before it persists the route in the NemoClaw registry or agent config, the command sends a validation request from the target sandbox.
The output limit is 16 tokens for ordinary providers and 256 tokens for Google Gemini.
When the switch changes the API family and that request returns HTTP `400` or `404`, NemoClaw retries up to two times after delays of 2 and 4 seconds.
...
If provider verification or that request fails, the command leaves or restores the previous OpenShell selection and removes the provider only when it can verify the revision created by this switch.
...
Provider updates for DNS-backed HTTPS routes occur after route selection.
...
If NemoClaw rejects an update before the provider command runs, the provider binding stays unchanged.
If a provider command fails without confirming that no change occurred, provider state may be partial.
...
Refer to [Switch Inference Providers](../inference/manage-inference/switch-providers#handle-compatible-endpoints) for the complete recovery procedure.
...
For a same-provider model change, omit the endpoint options so NemoClaw reuses the endpoint recorded for the target sandbox.
When you supply endpoint options, missing or `inference set` provenance and every different URL remain subject to the full address validation above.
...
<AgentOnly variant="hermes">
```bash
$$nemoclaw credentials add tavily-search --type tavily --agent hermes --credential TAVILY_API_KEY
```

For an existing incorrect binding, follow [Repair a Hermes Tavily Provider](../security/credential-rotation#repair-a-hermes-tavily-provider).
</AgentOnly>
<AgentOnly variant="openclaw">
```bash
$$nemoclaw credentials add tavily-search --type tavily --agent openclaw --credential TAVILY_API_KEY
```
</AgentOnly>
<AgentOnly variant="deepagents">
```bash
$$nemoclaw credentials add tavily-search --type tavily --agent dcode --credential TAVILY_API_KEY
```
</AgentOnly>

| Flag | Description |
| --- | --- |
| `--type <TYPE>` | Provider profile, such as `tavily`, `tavily-hermes-v1`, `nvidia`, `openai`, `anthropic`, or `generic` |
| `--agent <AGENT>` | Agent name or alias for Tavily profile selection, such as `hermes` or `dcode`. Does not select or rebuild a sandbox |
...
| `--destroy-user-data` | Skip eligible fresh sandbox backups and remove preserved user data (`rebuild-backups/`, `backups/`, `sandboxes.json`). Removes installer-managed user-local CLI shims under `~/.local/bin/` only when sibling evidence is unidentified. When a confirmed sibling gateway port remains, those shared shims stay with the shared npm CLI package. |
...
<AgentOnly variant="hermes">
  <Warning>
    For a Docker Hermes sandbox that uses the NemoClaw-managed image, uninstall removes
...
If a prior uninstall removed a non-default gateway but retained its registry and backups, rerun `NEMOCLAW_GATEWAY_PORT=<port> $$nemoclaw uninstall --destroy-user-data` to remove that retained data.
...
| --- | --- | --- |
| `NEMOCLAW_GATEWAY_PORT` | A recorded automatic alternate port, otherwise 8080 | OpenShell gateway port. An explicit supported value remains authoritative. See [Installer Reports That the Systemd User Manager Is Unavailable](./troubleshooting#installer-reports-that-the-systemd-user-manager-is-unavailable). |
...
| `NEMOCLAW_HERMES_DASHBOARD`     | 0       | Enable the optional Hermes dashboard      |
| `NEMOCLAW_HERMES_DASHBOARD_PORT` | 18789   | Hermes dashboard forward port; when set, it must match `NEMOCLAW_DASHBOARD_PORT` |
| `NEMOCLAW_HERMES_DASHBOARD_INTERNAL_PORT` | 19119 | Hermes dashboard backend port        |
...
| --- | --- | --- |
| `NEMOCLAW_DEFER_ONBOARDING` | `1` to enable | Hosted installer only. Selects the same behavior as `--defer-onboarding` for Hermes or LangChain Deep Agents Code with NVIDIA hosted inference. When none of the accepted credentials (`NVIDIA_INFERENCE_API_KEY`, `NVIDIA_API_KEY`, or a credential value in `NEMOCLAW_PROVIDER_KEY`) is present and no sandbox is registered, the installer installs NemoClaw without creating a provider, sandbox, or completed onboarding state. |
| `NEMOCLAW_PROVIDER` | provider key (e.g. `build`, `openrouter`, `openai`, `anthropic`, `anthropicCompatible`, `gemini`, `ollama`, `custom`, `vllm`, `nim-local`, `routed`, `hermes-provider`, `llama-cpp`, `install-llama-cpp`, `install-vllm`, `install-ollama`, `install-windows-ollama`, `start-windows-ollama`) | Selects the inference provider during onboarding. The wizard skips the provider menu in both interactive and non-interactive runs when this is set. On a qualified N1x host, `install-vllm` supplies Deferred preview intent, while another provider supplies standard-onboarding intent and retains its existing requirements. Local NVIDIA NIM remains unavailable on N1x. `llama-cpp` selects attachment of an authenticated, operator-managed llama.cpp server on loopback port `8081`. Set `NEMOCLAW_LLAMACPP_LOCAL_TOKEN`; set `NEMOCLAW_MODEL` to the served alias when the server exposes multiple models. `install-llama-cpp` selects the compatible experimental managed path on DGX Spark, generic NVIDIA GPU, or Windows WSL N1x hosts and rejects `NEMOCLAW_MODEL`; use `NEMOCLAW_LLAMACPP_RECIPE` for an explicit declarative selection. A qualifying Windows WSL N1x host automatically selects this provider and its Qwen 3.6 recipe when no provider is requested or recorded. If an operator-managed server does not provide consistent native llama.cpp evidence, select `custom`. Aliases: `cloud` → `build`, `open-router` / `openrouterai` → `openrouter`, `nim` → `nim-local`, `hermes` / `nous` / `nous-portal` → `hermes-provider`, `anthropiccompatible` → `anthropicCompatible`. Invalid values fail fast with the list of accepted keys. |
...
| `NEMOCLAW_OLLAMA_PROXY_SKIP_BIND_PROBE` | exactly `1` to enable | Starts the Ollama auth proxy even when the selected Ollama daemon or unauthenticated compatible endpoint answers on a non-loopback interface. The proxy otherwise refuses to start, because any host that can reach that backend can bypass the proxy's token. Binding the selected backend to loopback is the supported fix; use this override only when you accept that exposure. Export the variable rather than setting it for one command: a later NemoClaw command that respawns the proxy without it refuses to start again. |
...
export TELEGRAM_BOT_TOKEN=<primary-bot-token>
...
When NemoClaw creates the canonical provider, it omits an extension whose credential is unavailable from both the credential store and its environment variable.
The primary credential is required to create that provider.
An extension already stored in an existing provider remains available until that provider is replaced or removed.
If the extension credential is absent from the credential store, export it before onboarding.
...
<Warning title="Host-local Ollama is not supported">
...
| --- | --- | --- |
| `NEMOCLAW_MCP_PROVIDER_SYNC_TIMEOUT_SECONDS` | `30` | Maximum time to wait for an OpenShell MCP provider credential handle to become active or fully revoked inside the sandbox. Integer seconds; raise only when provider synchronization is unusually slow. |
...
| `NEMOCLAW_WSL_GPU_PROOF_TIMEOUT_MS` | `180000` | Maximum time for the selected provider's bounded CUDA workload on an eligible ARM64 Linux host. A positive integer overrides the default and is capped at `900000` (15 minutes). Invalid, infinite, zero, and negative values use the default. |
...
| `NEMOCLAW_GATEWAY_RECOVERY_WAIT_SECONDS` | `30`, `90`, or `120`, depending on the recovery phase | Wall-clock timeout for OpenShell command re-registration after policy application, gateway startup after an intentional OpenClaw stop, and gateway health and OpenShell readiness during managed or provider recovery. A valid finite, nonnegative value overrides the internal budget for the current recovery phase. |
...
After an intentional stop, `$$nemoclaw <sandbox-name> start` uses the 30-second OpenClaw gateway-health budget, including probe execution and delays.
`NEMOCLAW_GATEWAY_RECOVERY_WAIT_SECONDS` overrides this startup budget and preserves finite, nonnegative fractional seconds.
If the budget expires, `start` exits nonzero before gateway-health verification and host-forward restoration.

</AgentOnly>

<AgentOnly variant="openclaw,hermes">

For managed or provider recovery, `NEMOCLAW_GATEWAY_RECOVERY_WAIT_SECONDS` covers gateway health and post-recovery OpenShell readiness.
When the deadline expires, NemoClaw leaves the existing container identity unchanged and does not restore the primary dashboard or API host forward.
...
Set the recovery override before a `start` or `recover` command when gateway health or OpenShell readiness needs more time.
